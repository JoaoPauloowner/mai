...
export default function PublicQuizPage() {
  ...
  const inputClass =
    "w-full px-4 py-3 bg-white border border-[#E0E3DE] rounded-xl text-sm text-[#2C2E2A] focus:outline-none focus:border-[#7A8E75] focus:ring-1 focus:ring-[#7A8E75] transition";

  const progressPct = completed ? 100 : Math.round((step / 3) * 100);

  return (
    <div className="min-h-screen bg-[#F5F5F5] text-[#2C2E2A] flex flex-col justify-center items-center p-4 selection:bg-[#C1ED84] selection:text-[#2C2E2A]">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="font-serif text-3xl font-bold tracking-tight text-[#2C2E2A]">{quizConfig.titulo}</h1>
          <p className="text-sm text-[#63695B] mt-1 max-w-sm mx-auto">{quizConfig.subtitulo}</p>
        </div>

        {/* Card do Quiz */}
        <div className="bg-white border border-[#E0E3DE] rounded-2xl shadow-sm overflow-hidden">
          {/* Barra de Progresso */}
          <div className="h-1.5 bg-[#F5F5F5]">
            <div
              className="h-full bg-[#C1ED84] transition-all duration-500"
          ...
          <div className="p-7">
            {!completed ? (
              <div>
                ...
                {step === 1 && (
                  <div className="space-y-5">
                    <div>
                      <span className="text-[10px] font-mono text-[#7A8E75] uppercase tracking-wider block mb-1">
                        Etapa 1 de 3: Objetivo Principal
                      </span>
                      <h2 className="font-serif text-xl font-bold text-[#2C2E2A]">
                    ...
                    <div className="space-y-2.5">
                      {quizConfig.step1.opcoes.map((opcao) => (
                        <button
                          ...
                          className="w-full p-4 rounded-xl border text-left text-sm font-medium transition-all hover:border-[#7A8E75] hover:bg-[#F5F5F5] active:bg-[#E7EBE6] border-[#E0E3DE] text-[#2C2E2A]"
                        >
                          {opcao}
                        </button>
                      ))}
                ...
                {step === 3 && (
                  <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div className="space-y-3">
                      <div>
                        <label className="block text-xs font-bold text-[#2C2E2A] mb-1.5">Seu Nome Completo</label>
                        <input
                          type="text"
                          required
                          value={nome}
                      ...
                      <div>
                        <label className="block text-xs font-bold text-[#2C2E2A] mb-1.5">WhatsApp com DDD</label>
                        <input
                          type="tel"
                          required
                          value={telefone}
                      ...
                      <div>
                        <label className="block text-xs font-bold text-[#2C2E2A] mb-1.5">E-mail (opcional)</label>
                        <input
                          type="email"
                          value={email}
                    ...
                    <button
                      type="submit"
                      disabled={loading}
                      className="w-full py-4 rounded-xl bg-[#C1ED84] hover:bg-[#B2E372] text-[#2C2E2A] font-bold text-sm transition border border-[#A5DC60] flex items-center justify-center gap-2 disabled:opacity-50 shadow-xs"
                    >
                      {loading ? (
                        <>
              ...
              <div className="text-center py-6 space-y-5">
                <div className="w-16 h-16 rounded-full bg-[#DDE8DE] border-2 border-[#C4D7C4] text-[#2D6A4F] mx-auto flex items-center justify-center">
                  <CheckCircle2 className="w-8 h-8" />
                </div>

                <div>
                  <h2 className="font-serif text-2xl font-bold text-[#2C2E2A]">
                    Solicitação Recebida!
                  </h2>
                  <p className="text-sm text-[#63695B] max-w-sm mx-auto mt-2 leading-relaxed">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-[#2D6A4F] hover:bg-[#24543E] text-white font-bold text-sm transition shadow-sm"
                >
            ...
            <div className="mt-6 pt-4 border-t border-[#E0E3DE] flex items-center justify-center gap-2 text-[10px] text-[#7C8472]">

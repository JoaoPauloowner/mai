...
export default async function CampanhasPage() {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto text-[#0F172A]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Campanhas UTM */}
        <div className="p-6 rounded-2xl bg-white border border-[#E2E8F0] space-y-4 shadow-xs">
          <h3 className="text-sm font-bold text-[#0F172A] flex items-center justify-between">
            <span className="flex items-center gap-2">
              <Target className="w-4 h-4 text-[#2563EB]" /> Campanhas Tráfego Pago (UTMs)
            </span>
            <span className="text-[10px] font-mono text-[#64748B] uppercase">Google & Meta Ads</span>
          </h3>

          <div className="space-y-3">
            {campaigns.length === 0 ? (
              <div className="p-8 text-center text-xs text-[#64748B] bg-[#F8FAFC] rounded-xl border border-[#E2E8F0]">
              ...
              campaigns.map((c, idx) => (
                ...
                >
                  <div>
                    <div className="font-bold text-[#0F172A]">{c.utmCampaign}</div>
                    <div className="text-[10px] text-[#64748B] font-mono mt-0.5">
                      Fonte: {c.utmSource || "Direto"}
                    </div>
                  </div>
                  <span className="px-2.5 py-1 rounded-full bg-[#EFF6FF] text-[#2563EB] font-mono font-bold text-[11px]">
                    {c._count.id} leads
                  </span>
                </div>
              ))
        ...
        <div className="p-6 rounded-2xl bg-white border border-[#E2E8F0] space-y-4 shadow-xs">
          <h3 className="text-sm font-bold text-[#0F172A] flex items-center justify-between">
            <span className="flex items-center gap-2">
              <InstagramIcon className="w-4 h-4 text-[#2563EB]" /> Palavras-Chave Instagram Direct
            </span>
            <span className="text-[10px] font-mono text-[#64748B] uppercase">Gatilhos Automáticos</span>
          </h3>
            ...
            {directKeywords.length === 0 ? (
              ...
              directKeywords.map((k, idx) => (
                <div
                  ...
                  className="p-3.5 rounded-xl bg-[#F8FAFC] border border-[#E2E8F0] flex items-center justify-between text-xs"
                >
                  <div className="font-mono font-bold text-[#0F172A]">#{k.directKeyword}</div>
                  <span className="px-2.5 py-1 rounded-full bg-[#EFF6FF] text-[#2563EB] font-mono font-bold text-[11px]">

        ...
        <CardContent>
          <form onSubmit={handleSimularAvaliacao} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-xs font-bold text-[#0F172A] mb-1">Placa do Veículo</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3.5 py-2.5 bg-white border border-[#E2E8F0] rounded-xl text-xs font-mono font-bold text-[#0F172A] uppercase focus:outline-none focus:border-[#2563EB]"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-[#0F172A] mb-1">Marca / Modelo / Versão</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3.5 py-2.5 bg-white border border-[#E2E8F0] rounded-xl text-xs text-[#0F172A] focus:outline-none focus:border-[#2563EB]"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-[#0F172A] mb-1">Ano / Quilometragem</label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-[#0F172A] mb-1">Nome do Cliente</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3.5 py-2.5 bg-white border border-[#E2E8F0] rounded-xl text-xs text-[#0F172A]"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-[#0F172A] mb-1">WhatsApp de Contato</label>
                <input
                  type="text"
                  required
              ...
              <div key={rec.id} className="py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-sm text-[#0F172A]">{rec.veiculo}</span>
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-[#F8FAFC] text-[#0F172A] font-bold">
                      {rec.ano} • {rec.km}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {rec.avarias.map((av, i) => (
                      <span key={i} className="text-[10px] bg-[#F8FAFC] border border-[#E2E8F0] px-2 py-0.5 rounded text-[#0F172A]">
                        ✓ {av}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex items-center gap-4 text-right shrink-0">
                  <div>
                    <span className="text-[10px] text-[#64748B] uppercase font-mono block">Tabela FIPE</span>
                    <span className="text-xs font-mono text-[#64748B] line-through">
                      R$ {rec.valorFipe.toLocaleString("pt-BR")}
                    </span>
                    <span className="text-[10px] text-[#64748B] uppercase font-mono block mt-1">Oferta Concessionária</span>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="px-3.5 py-2 rounded-xl bg-[#2563EB] text-[#0F172A] font-bold text-xs hover:bg-[#1D4ED8] transition border border-[#A5DC60]"
                  >
                    Enviar Proposta
                  </a>
                </div>

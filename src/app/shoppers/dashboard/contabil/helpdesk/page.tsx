    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="p-4 rounded-2xl bg-white border border-[#E2E8F0] shadow-xs">
          <div className="text-[10px] text-[#64748B] uppercase font-mono mb-1">Fiscal / Tributário</div>
          <div className="text-2xl font-serif font-bold text-[#0F172A]">12 chamados</div>
          <p className="text-[11px] text-[#2563EB] mt-1">SLA Médio: 18 min</p>
        </div>
        <div className="p-4 rounded-2xl bg-white border border-[#E2E8F0] shadow-xs">
          <div className="text-[10px] text-[#64748B] uppercase font-mono mb-1">Dep. Pessoal (DP)</div>
          <div className="text-2xl font-serif font-bold text-[#0F172A]">8 chamados</div>
          <p className="text-[11px] text-[#2563EB] mt-1">Admissões & Rescisões</p>
        </div>
        <div className="p-4 rounded-2xl bg-white border border-[#E2E8F0] shadow-xs">
          <div className="text-[10px] text-[#64748B] uppercase font-mono mb-1">Societário / Legal</div>
          <div className="text-2xl font-serif font-bold text-[#0F172A]">4 chamados</div>
          <p className="text-[11px] text-[#64748B] mt-1">JUCESP & Alvarás</p>
        </div>
        <div className="p-4 rounded-2xl bg-white border border-[#E2E8F0] shadow-xs">
          <div className="text-[10px] text-[#64748B] uppercase font-mono mb-1">Satisfação Clientes</div>
          <div className="text-2xl font-serif font-bold text-[#2563EB]">98.4%</div>
          <p className="text-[11px] text-[#2563EB] mt-1">Avaliações WhatsApp 5★</p>
        </div>
      </div>
              ...
              <div key={c.id} className="py-4 flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-[10px] font-bold bg-[#F8FAFC] text-[#0F172A] px-2 py-0.5 rounded border border-[#CBD5E1]">
                      {c.departamento}
                    </span>
                    <span className="font-bold text-sm text-[#0F172A]">{c.cliente}</span>
                    <span
                      className={`text-[9px] font-mono px-2 py-0.5 rounded-full font-bold border ${
                        c.status === "RESOLVIDO"
                          ? "bg-[#EFF6FF] text-[#2563EB] border-[#C8E5D1]"
                          : c.status === "EM_ATENDIMENTO"
                          ? "bg-[#2563EB] text-[#0F172A] border-[#A5DC60]"
                  ...
                  <p className="font-bold text-xs text-[#0F172A]">{c.assunto}</p>
                  <p className="text-[#64748B] text-xs italic">&ldquo;{c.ultimaMensagem}&rdquo;</p>
                </div>

                <div className="flex items-center gap-3 shrink-0">
                  <div className="text-right text-xs">
                    <span className="text-[10px] text-[#64748B] uppercase font-mono block">Tempo na Fila</span>
                    <span className="font-mono font-bold text-[#0F172A]">{c.tempoEspera}</span>
                  </div>
